...
it('shows the seven-day next declaration time and disables the action', async () => { localStorage.setItem('accessToken','x'); render(<MemoryRouter><Routes><Route path="*" element={<PassportPage />} /></Routes></MemoryRouter>); expect(await screen.findByText(/Bạn có thể khai báo lại từ/)).toBeInTheDocument(); expect(screen.getByRole('button',{name:'Khai báo trình độ'})).toBeDisabled() })
