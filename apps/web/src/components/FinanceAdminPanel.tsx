...
type WithdrawalAction = { id: string; kind: 'reject' | 'finalize' };
    ...
    <section>
      ...
      {pageRows.length === 0 ? <div className="mt-4"><EmptyState title="Không có yêu cầu phù hợp" description="Thử thay đổi từ khóa, trạng thái hoặc cách sắp xếp." /></div> : <div className="mt-4 space-y-3">{(pageRows as WithdrawalRow[]).map((row) => {
        ...
        return <article className="rounded-xl border border-line bg-surface p-4 shadow-sm" key={row.id}>
          ...
          {row.status === 'pending' || row.status === 'partially_paid' ? <div className="mt-4">{withdrawalAction?.id === row.id ? <div className="grid gap-3 rounded-xl border border-danger/25 bg-danger-bg/40 p-3"><label className="grid gap-1 text-sm font-medium">{withdrawalAction.kind === 'reject' ? 'Lý do từ chối' : 'Lý do chốt mức đã chi'}<TextInput aria-label={`${withdrawalAction.kind === 'reject' ? 'Lý do từ chối' : 'Lý do chốt mức đã chi'} ${row.transferCode}`} value={withdrawalReasons[row.id] ?? ''} onChange={(event) => setWithdrawalReasons({ ...withdrawalReasons, [row.id]: event.target.value })} placeholder="Bắt buộc trước khi xác nhận" autoFocus /></label><div className="flex flex-wrap gap-2"><Button tone="danger" size="sm" onClick={() => requestConfirmation(withdrawalAction.kind === 'reject' ? 'Từ chối yêu cầu rút tiền' : 'Chốt mức đã chi', withdrawalReasons[row.id] ?? '', () => withdrawalAction.kind === 'reject' ? rejectWithdrawal(row.id, withdrawalReasons[row.id] ?? '') : finalizePartialWithdrawal(row.id, withdrawalReasons[row.id] ?? ''))}>{withdrawalAction.kind === 'reject' ? 'Xác nhận từ chối' : 'Xác nhận chốt mức đã chi'}</Button><Button tone="ghost" size="sm" onClick={() => setWithdrawalAction(null)}>Hủy</Button></div></div> : <div className="flex flex-wrap gap-2">{row.status === 'pending' ? <Button tone="danger" size="sm" onClick={() => setWithdrawalAction({ id: row.id, kind: 'reject' })}>Từ chối</Button> : null}{row.status === 'partially_paid' ? <Button tone="danger" size="sm" onClick={() => setWithdrawalAction({ id: row.id, kind: 'finalize' })}>Chốt mức đã chi</Button> : null}</div>}</div> : null}
        </article>;
      })}</div>}
