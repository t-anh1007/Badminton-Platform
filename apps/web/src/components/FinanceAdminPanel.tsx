...
import { confirmManualWithdrawalPayout, finalizePartialWithdrawal, getAdminWithdrawals, getReconciliationQueue, markOutOfScope, reconcileIncoming, reconcileOutgoing, rejectWithdrawal, type ReconciliationRow, type WithdrawalRow } from '../lib/financeApi';
...
type ReconciliationAction = { id: string; kind: 'incoming' | 'outgoing' | 'outOfScope' };
...
const withdrawalQr = (row: WithdrawalRow) => {
  const amount = (BigInt(row.amount) - BigInt(row.paidAmount)).toString();
  const query = new URLSearchParams({
    acc: row.bankAccountNumber,
    bank: row.bankCode,
    amount,
    des: row.transferCode,
    template: 'compact',
  });
  return {
    ...
    amount,
    matchCode: row.transferCode,
    qrImageUrl: `https://qr.sepay.vn/img?${query.toString()}`,
  };
};
...
export function FinanceAdminPanel({ mode }: { mode: 'withdrawals' | 'reconciliation' }) {
  ...
  const [reconciliationTargets, setReconciliationTargets] = useState<Record<string, string>>({});
  const [reconciliationReasons, setReconciliationReasons] = useState<Record<string, string>>({});
  const [qrWithdrawal, setQrWithdrawal] = useState<WithdrawalRow | null>(null);
  ...
  const reload = async () => {
    try {
      if (mode === 'withdrawals') {
        ...
        const [reconciliationRows, withdrawalRows] = await Promise.all([getReconciliationQueue(), getAdminWithdrawals()]);
        setEvents(reconciliationRows);
        setWithdrawals(withdrawalRows);
        const identities = await getAdminAccountIdentities([...new Set(withdrawalRows.map((row) => row.sellerUserId))]);
        setAccountIdentities(new Map(identities.map((identity) => [identity.id, identity])));
      }
    } catch (error) {
  ...
  const requestConfirmation = (label: string, actionReason: string, task: () => Promise<unknown>) => {
    if (!actionReason.trim()) { setMessage('Nhập lý do trước khi xác nhận thao tác này.'); return; }
    setPending({ label, reason: actionReason.trim(), task });
  };
  ...
  const content = mode === 'withdrawals' ? (
    <section>
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2"><p className="text-sm text-ink-500">Hiển thị <strong className="text-ink-900">{filteredWithdrawals.length}</strong> / {withdrawals.length} yêu cầu</p><p className="text-xs text-ink-500">Lý do chỉ được nhập khi xử lý từng yêu cầu.</p></div>
      {pageRows.length === 0 ? <div className="mt-4"><EmptyState title="Không có yêu cầu phù hợp" description="Thử thay đổi từ khóa, trạng thái hoặc cách sắp xếp." /></div> : <div className="mt-4 space-y-3">{(pageRows as WithdrawalRow[]).map((row) => {
        ...
        return <article className="rounded-xl border border-line bg-surface p-4 shadow-sm" key={row.id}>
          <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-ink-500">Nội dung chuyển khoản</p><p className="mt-1 text-figures text-lg font-bold text-brand-navy">{row.transferCode}</p><p className="mt-1 text-xs font-semibold text-ink-500">{row.walletType === 'personal' ? 'Ví người chơi' : 'Ví chủ sân'}</p></div><Badge tone={withdrawalStatusTone(row.status)}>{withdrawalStatusLabel(row.status)}</Badge></div>
          ...
          {row.status === 'pending' || row.status === 'partially_paid' ? <div className="mt-4">{withdrawalAction?.id === row.id ? <div className="grid gap-3 rounded-xl border border-danger/25 bg-danger-bg/40 p-3"><label className="grid gap-1 text-sm font-medium">{withdrawalAction.kind === 'reject' ? 'Lý do từ chối' : withdrawalAction.kind === 'manual' ? 'Mã tham chiếu/lý do chi' : 'Lý do chốt mức đã chi'}<TextInput aria-label={`${withdrawalAction.kind === 'reject' ? 'Lý do từ chối' : withdrawalAction.kind === 'manual' ? 'Mã tham chiếu/lý do chi' : 'Lý do chốt mức đã chi'} ${row.transferCode}`} value={withdrawalReasons[row.id] ?? ''} onChange={(event) => setWithdrawalReasons({ ...withdrawalReasons, [row.id]: event.target.value })} placeholder="Bắt buộc trước khi xác nhận" autoFocus /></label><div className="flex flex-wrap gap-2"><Button tone={withdrawalAction.kind === 'reject' ? 'danger' : 'primary'} size="sm" onClick={() => requestConfirmation(withdrawalAction.kind === 'reject' ? 'Từ chối yêu cầu rút tiền' : withdrawalAction.kind === 'manual' ? 'Ghi nhận đã chuyển tiền ngoài SePay' : 'Chốt mức đã chi', withdrawalReasons[row.id] ?? '', () => withdrawalAction.kind === 'reject' ? rejectWithdrawal(row.id, withdrawalReasons[row.id] ?? '') : withdrawalAction.kind === 'manual' ? confirmManualWithdrawalPayout(row.id, withdrawalReasons[row.id] ?? '') : finalizePartialWithdrawal(row.id, withdrawalReasons[row.id] ?? ''))}>{withdrawalAction.kind === 'reject' ? 'Xác nhận từ chối' : withdrawalAction.kind === 'manual' ? 'Xác nhận đã chuyển tiền' : 'Xác nhận chốt mức đã chi'}</Button><Button tone="ghost" size="sm" onClick={() => setWithdrawalAction(null)}>Hủy</Button></div></div> : <div className="flex flex-wrap gap-2"><Button size="sm" aria-label={`Hiển thị mã QR ${row.transferCode}`} onClick={() => setQrWithdrawal(row)}>Hiển thị mã QR</Button><Button tone="secondary" size="sm" aria-label={`Đã chuyển tiền ${row.transferCode}`} onClick={() => setWithdrawalAction({ id: row.id, kind: 'manual' })}>Đã chuyển tiền</Button>{row.status === 'pending' ? <Button tone="danger" size="sm" onClick={() => setWithdrawalAction({ id: row.id, kind: 'reject' })}>Từ chối</Button> : null}{row.status === 'partially_paid' ? <Button tone="danger" size="sm" onClick={() => setWithdrawalAction({ id: row.id, kind: 'finalize' })}>Chốt mức đã chi</Button> : null}</div>}</div> : null}
        </article>;
      })}</div>}
    ...
    <section>
      ...
      {pageRows.length === 0 ? <div className="mt-4"><EmptyState title="Không có giao dịch phù hợp" description="Thử thay đổi từ khóa, hướng tiền hoặc cách sắp xếp." /></div> : <div className="mt-4 grid gap-3 lg:grid-cols-2">{(pageRows as ReconciliationRow[]).map((event) => {
        const action = reconciliationAction?.id === event.id ? reconciliationAction : null;
        const reasonForEvent = reconciliationReasons[event.id] ?? '';
        const matchingWithdrawals = withdrawals.filter((row) => (row.status === 'pending' || row.status === 'partially_paid') && BigInt(row.amount) - BigInt(row.paidAmount) === BigInt(event.amount));
        const submit = () => action?.kind === 'incoming'
          ? requestConfirmation('Gán giao dịch vào ví cá nhân', reasonForEvent, () => reconcileIncoming(event.id, reconciliationTargets[event.id] ?? '', reasonForEvent))
          : action?.kind === 'outgoing'
            ? requestConfirmation('Gán giao dịch vào yêu cầu rút', reasonForEvent, () => reconcileOutgoing(event.id, reconciliationTargets[event.id] ?? '', reasonForEvent))
            : requestConfirmation('Đánh dấu giao dịch ngoài phạm vi', reasonForEvent, () => markOutOfScope(event.id, reasonForEvent));
        const needsTarget = action?.kind === 'incoming' || action?.kind === 'outgoing';
        return <article className="rounded-xl border border-line bg-surface p-4" key={event.id}><div className="flex items-start justify-between gap-3"><div><p className="text-xs text-ink-500">{event.direction === 'in' ? 'Tiền vào' : 'Tiền ra'}</p><p className="text-figures text-xl font-bold text-ink-900">{formatMoneyVnd(event.amount)}</p></div><Badge tone={event.direction === 'in' ? 'success' : 'warning'}>{event.direction === 'in' ? 'Tiền vào' : 'Tiền ra'}</Badge></div><p className="mt-3 text-sm text-ink-500">Nhận lúc <span className="text-ink-900">{formatDateTimeVi(event.receivedAt)}</span></p><details className="mt-2 text-xs text-ink-500"><summary className="cursor-pointer font-medium">Xem tham chiếu đối soát</summary><code className="mt-2 block break-all rounded-lg bg-canvas p-2">{event.rawRef || shortReference(event.id)}</code></details>{action ? <div className="mt-4 grid gap-3 rounded-xl border border-brand-yellow/50 bg-warning-bg/40 p-3"><p className="font-semibold text-ink-900">{action.kind === 'incoming' ? 'Gán với ví cá nhân' : action.kind === 'outgoing' ? 'Gắn với yêu cầu rút' : 'Đánh dấu ngoài phạm vi'}</p>{action.kind === 'incoming' ? <label className="grid gap-1 text-sm font-medium">Chọn người nhận<SelectInput aria-label={`Chọn người nhận ${event.id}`} value={reconciliationTargets[event.id] ?? ''} onChange={(input) => setReconciliationTargets({ ...reconciliationTargets, [event.id]: input.target.value })}><option value="">Chọn tài khoản</option>{[...accountIdentities.values()].map((identity) => <option key={identity.id} value={identity.id}>{identity.displayName || identity.email}</option>)}</SelectInput></label> : null}{action.kind === 'outgoing' ? <label className="grid gap-1 text-sm font-medium">Chọn yêu cầu rút tiền<SelectInput aria-label={`Chọn yêu cầu rút ${event.id}`} value={reconciliationTargets[event.id] ?? ''} onChange={(input) => setReconciliationTargets({ ...reconciliationTargets, [event.id]: input.target.value })}><option value="">Chọn yêu cầu rút phù hợp</option>{matchingWithdrawals.map((row) => <option key={row.id} value={row.id}>{row.transferCode} · {accountIdentities.get(row.sellerUserId)?.displayName || accountIdentities.get(row.sellerUserId)?.email || row.bankAccountName} · {formatMoneyVnd(BigInt(row.amount) - BigInt(row.paidAmount))}</option>)}</SelectInput>{matchingWithdrawals.length === 0 ? <span className="text-xs text-danger">Không có yêu cầu rút đang chờ có số tiền khớp.</span> : null}</label> : null}<label className="grid gap-1 text-sm font-medium">Lý do đối soát *<TextInput aria-label={`Lý do đối soát ${event.id}`} value={reasonForEvent} onChange={(input) => setReconciliationReasons({ ...reconciliationReasons, [event.id]: input.target.value })} placeholder="Bắt buộc trước khi xác nhận" autoFocus /></label><div className="flex flex-wrap gap-2"><Button size="sm" disabled={needsTarget && !reconciliationTargets[event.id]} onClick={submit}>{action.kind === 'incoming' ? 'Xác nhận gán ví cá nhân' : action.kind === 'outgoing' ? 'Xác nhận gán yêu cầu rút' : 'Xác nhận ngoài phạm vi'}</Button><Button tone="ghost" size="sm" onClick={() => setReconciliationAction(null)}>Hủy</Button></div></div> : <div className="mt-4 flex flex-wrap gap-2">{event.direction === 'in' ? <Button size="sm" onClick={() => setReconciliationAction({ id: event.id, kind: 'incoming' })}>Gán ví cá nhân</Button> : <Button size="sm" onClick={() => setReconciliationAction({ id: event.id, kind: 'outgoing' })}>Gán yêu cầu rút</Button>}<Button tone="danger" size="sm" onClick={() => setReconciliationAction({ id: event.id, kind: 'outOfScope' })}>Ngoài phạm vi</Button></div>}</article>;
      })}</div>}
      {pageCount > 1 ? <div className="mt-5"><Pagination page={page} pageCount={pageCount} onChange={setPage} /></div> : null}
    </section>
  );

  return <>{content}{message && <p role="status" className="mt-4 text-sm text-ink-500">{message}</p>}<Modal open={Boolean(qrWithdrawal)} title="Quét QR chuyển tiền" onClose={() => setQrWithdrawal(null)}>{qrWithdrawal ? <div className="space-y-3"><SepayPayBox payment={withdrawalQr(qrWithdrawal)} /><p className="rounded-lg bg-warning-bg p-3 text-sm text-ink-700">Quét QR chỉ điền sẵn thông tin chuyển tiền. Yêu cầu chỉ được xác nhận sau khi SePay nhận webhook tiền ra khớp đúng số tiền và nội dung.</p></div> : null}</Modal><Modal open={Boolean(pending)} title="Xác nhận thao tác không thể đảo ngược" onClose={() => setPending(null)}><p className="text-sm text-ink-500">{pending?.label}. Lý do sẽ được lưu cùng audit: <strong className="text-ink-900">{pending?.reason}</strong></p><Button tone="danger" className="mt-5" onClick={() => void confirm()}>Xác nhận</Button></Modal></>;
}
