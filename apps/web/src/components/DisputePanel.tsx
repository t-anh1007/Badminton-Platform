...
export function DisputePanel() {
  ...
  return (
    <section className="mt-10" aria-labelledby="dispute-title">
      ...
      <form onSubmit={submit} className="grid gap-3 rounded-2xl bg-surface p-4">
        ...
        <button type="submit" className="w-fit rounded-full bg-green-600 px-4 py-2 text-surface">Gửi tranh chấp</button>
      </form>
      <h3 className="mt-5 font-semibold">Tranh chấp của tôi</h3>
      <div className="mt-2 space-y-2">{disputes.map((row) => <div key={row.id} className="rounded-xl bg-surface p-3">
