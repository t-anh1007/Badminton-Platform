...
function DisputeDecisionCard({ row, onRequestDecision }: { row: DisputeRow; onRequestDecision: (decision: PendingDecision) => void }) {
  ...
  return <article className="rounded-xl border border-line bg-surface p-4"><p className="font-medium">{row.bookingId} · {row.status} · hạn {new Date(row.deadlineAt).toLocaleString('vi-VN')}</p><p className="mt-1 text-sm text-ink-500">{row.reason}</p>{row.revenue && <p className="mt-2 text-sm text-ink-500">Booking: gộp {money(row.revenue.gross)} · ròng {money(row.revenue.net)} · hoa hồng {money(row.revenue.commission)}</p>}{row.evidence?.length > 0 && <ul className="mt-2 text-sm text-ink-500">{row.evidence.map((item) => <li key={item}>{item}</li>)}</ul>}{row.ledgerEntries?.length ? <details className="mt-2 text-sm"><summary>Lịch sử bút toán</summary><ul>{row.ledgerEntries.map((entry) => <li key={entry.id}>{entry.type} · {entry.wallet.walletType} · {money(entry.amount)} · {entry.before} → {entry.after}</li>)}</ul></details> : null}{row.status === 'open' && <div className="mt-4 grid gap-2"><TextInput aria-label={`Số tiền hoàn một phần ${row.bookingId}`} value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="Số tiền hoàn một phần" /><TextInput aria-label={`Lý do quyết định ${row.bookingId}`} value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Lý do bắt buộc" /><div className="flex flex-wrap gap-3"><Button size="sm" onClick={() => onRequestDecision({ id: row.id, decision: 'full_refund', amount, reason })}>Hoàn toàn bộ</Button><Button size="sm" onClick={() => onRequestDecision({ id: row.id, decision: 'partial_refund', amount, reason })}>Hoàn một phần</Button><Button tone="danger" size="sm" onClick={() => onRequestDecision({ id: row.id, decision: 'rejected', amount, reason })}>Bác tranh chấp</Button></div></div>}{row.resolutionAmount && <p className="mt-2 text-sm text-ink-500">Đã hoàn {money(row.resolutionAmount)}</p>}</article>;
}
