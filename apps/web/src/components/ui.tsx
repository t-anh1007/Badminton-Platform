import { useEffect, useRef, useState, type ButtonHTMLAttributes, type InputHTMLAttributes, type ReactNode, type RefObject, type SelectHTMLAttributes, type TextareaHTMLAttributes } from 'react';
...
export function Button({ children, className = '', tone = 'primary', size = 'md', type = 'button', ...props }: ButtonHTMLAttributes<HTMLButtonElement> & { tone?: keyof typeof buttonTone; size?: keyof typeof buttonSize }) {
  return <button type={type} className={`inline-flex items-center justify-center gap-2 rounded-full font-bold uppercase tracking-[0.035em] transition duration-150 enabled:hover:-translate-y-px disabled:cursor-not-allowed disabled:opacity-50 ${buttonTone[tone]} ${buttonSize[size]} ${className}`} {...props}>{children}</button>;
}
...
export function Modal({ open, title, children, onClose, initialFocusRef }: { open: boolean; title: string; children: ReactNode; onClose: () => void; initialFocusRef?: RefObject<HTMLElement | null> }) {
  ...
  if (!open) return null;
  return <div className="fixed inset-0 z-[100] grid place-items-center bg-brand-navy/65 p-4" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="dialog-title" className="max-h-[calc(100vh-2rem)] w-full max-w-2xl overflow-y-auto rounded-2xl border border-line bg-surface shadow-[var(--shadow-raised)]"><div className="flex items-center justify-between border-b border-line px-5 py-4"><h2 id="dialog-title" className="text-h3">{title}</h2><button type="button" onClick={onClose} aria-label="Đóng hộp thoại" className="grid h-9 w-9 place-items-center rounded-full text-ink-500 hover:bg-canvas hover:text-brand-navy">×</button></div><div className="p-5 sm:p-6">{children}</div></div></div>;
}
...
export function EmptyState({ title, description, action }: { title: string; description: string; action?: ReactNode }) { return <div className="surface-card flex flex-col items-center p-8 text-center"><div aria-hidden className="mb-3 grid h-12 w-12 place-items-center rounded-full bg-green-50 text-xl text-brand-navy">⌁</div><h2 className="text-h3">{title}</h2><p className="mt-2 max-w-md text-sm text-ink-500">{description}</p>{action && <div className="mt-5">{action}</div>}</div>; }
