...
export function Modal({ open, title, children, onClose, initialFocusRef, dismissible = true }: { open: boolean; title: string; children: ReactNode; onClose: () => void; initialFocusRef?: RefObject<HTMLElement | null>; dismissible?: boolean }) {
  const titleId = useId();
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape') { if (dismissible) onCloseRef.current(); return; }
  ...
  if (!open) return null;
  return <div className="fixed inset-0 z-[1000] grid place-items-center bg-brand-navy/65 p-4" role="presentation" onMouseDown={(event) => { if (dismissible && event.target === event.currentTarget) onClose(); }}><div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby={titleId} className="max-h-[calc(100vh-2rem)] w-full max-w-2xl overflow-y-auto rounded-2xl border border-line bg-surface shadow-[var(--shadow-raised)]"><div className="flex items-center justify-between border-b border-line px-5 py-3"><h2 id={titleId} className="text-h3">{title}</h2>{dismissible && <button type="button" onClick={onClose} aria-label="Đóng hộp thoại" className="grid h-11 w-11 place-items-center rounded-full text-ink-500 hover:bg-canvas hover:text-brand-navy">×</button>}</div><div className="p-4 sm:p-5">{children}</div></div></div>;
}
...
export function Avatar({ label = 'T', src, alt = 'Ảnh đại diện', className = '' }: { label?: string; src?: string | null; alt?: string; className?: string }) { return src ? <img src={src} alt={alt} className={`h-9 w-9 rounded-full object-cover ${className}`} /> : <span aria-label="Tài khoản" className={`grid h-9 w-9 place-items-center rounded-full bg-brand-yellow text-sm font-bold text-brand-navy ${className}`}>{label.slice(0, 1).toUpperCase()}</span>; }
