...
import { getAdminAccountIdentities, listAdminAccounts, lockAccount, unlockAccount } from '../domain/adminAccounts.js';
...
adminRouter.post('/users/identities', requireAuth, requireRole('admin'), h(async (req, res) => {
  const { userIds } = z.object({ userIds: z.array(z.string().uuid()).max(200) }).strict().parse(req.body);
  res.json(await getAdminAccountIdentities([...new Set(userIds)]));
}));
