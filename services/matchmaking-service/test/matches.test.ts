...
function context(bookingId: string, startAt: Date): VenueMatchContext {
  return {
    ...
    venue: { id: randomUUID(), name: 'Sân Cầu Lông Quận 1', address: 'Quận 1', lat: 10.77, lng: 106.7 },
  };
}
...
beforeEach(async () => {
  if (eventAggregateIds.length > 0) {
    await prisma.outbox.deleteMany({ where: { aggregateId: { in: eventAggregateIds.splice(0) } } });
  }
  if (createdMatchIds.length > 0) {
    await prisma.outbox.deleteMany({ where: { aggregateId: { in: createdMatchIds.splice(0) } } });
  }
  if (bookingIds.length > 0) {
    const ids = bookingIds.splice(0);
    const matches = await prisma.match.findMany({ where: { bookingId: { in: ids } }, select: { id: true } });
    await prisma.join.deleteMany({ where: { matchId: { in: matches.map((match) => match.id) } } });
...
afterAll(async () => {
  await prisma.outbox.deleteMany({ where: { aggregateId: { in: eventAggregateIds } } });
  if (createdMatchIds.length > 0) {
    await prisma.outbox.deleteMany({ where: { aggregateId: { in: createdMatchIds } } });
  }
  if (bookingIds.length > 0) {
    const matches = await prisma.match.findMany({ where: { bookingId: { in: bookingIds } }, select: { id: true } });
    await prisma.join.deleteMany({ where: { matchId: { in: matches.map((match) => match.id) } } });
    await prisma.match.deleteMany({ where: { bookingId: { in: bookingIds } } });
  }
  await prisma.passport.deleteMany({ where: { userId: { in: passportUserIds } } });
...
describe('MMP-01 — public match search', () => {
  ...
  it('AC-MMP-01-2: filters by intersecting skill tier', async () => {
    const expected = await createMatch({ skillMin: 'intermediate', skillMax: 'advanced' });
  ...
  it('AC-MMP-01-3: excludes matches at or past cutoffAt', async () => {
    const visible = await createMatch({ cutoffAt: new Date(Date.now() + 60_000) });
...
describe('MMP-02 — create and publish a match', () => {
  it('AC-MMP-02-1: creates a split-fee match from the organizer owned held booking', async () => {
    ...
    venueBookingClient.contexts.set(bookingId, { ...slot, ownerUserId: organizerUserId });
...
describe('MMP-03 — public match detail', () => {
  async function detailFixture() {
    const match = await createMatch({ skillMin: 'intermediate', skillMax: 'advanced' });
    ...
    await prisma.join.createMany({
      data: [
        { matchId: match.id, participantUserId: randomUUID(), status: 'confirmed' },
        { matchId: match.id, participantUserId: randomUUID(), status: 'pending' },
      ],
    });
    return match;
  }
  ...
  it('AC-MMP-03-1: returns venue, time, fee, open slots and organizer tier', async () => {
    ...
    expect(response.body).toMatchObject({
      ...
      feePerSlot: '100000',
      openSlots: 2,
      organizer: { displayName: 'Nguyễn Minh', identityVisibility: 'public', tier: 'intermediate_plus' },
      court: { name: 'Sân 1' },
  ...
  it('AC-MMP-03-2: guest can view detail but cannot join', async () => {
    ...
    expect(response.body.actions).toEqual({ canJoin: false });
  });
});
...
describe('MMP-05 — organizer join review', () => {
  ...
  it('AC-MMP-05-3: an unpaid approval returns to pending after ten minutes', async () => {
    ...
    await prisma.join.update({
      where: { id: join.id },
      data: { status: 'approved', approvedAt: new Date('2026-08-08T00:00:00.000Z') },
    });
...
describe('MMP-06 — free match confirmation', () => {
  ...
  it('AC-MMP-06-3: concurrent free approvals cannot exceed capacity', async () => {
    ...
    const responses = await Promise.all(joins.map((join) => request(app)
      .post(`/matches/${match.id}/joins/${join.id}/approve`)
      .set('Authorization', `Bearer ${token}`)));
...
describe('join expiry scheduler lifecycle', () => {
  it('waits for an in-flight sweep before shutdown completes', async () => {
    ...
    const blockedSweep = new Promise<void>((resolve) => { releaseSweep = resolve; });
